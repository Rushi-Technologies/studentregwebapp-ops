pipeline {
    ...
    stages {
        stage('Git Clone') {
            steps {
                git branch: 'main', credentialsId: 'GitHubCred', url: 'https://github.com/Rushi-Technologies/studentregwebapp-ops.git'
            }
        }
        ...
        stage('Ping Tomcat Server') {
            steps {
                withCredentials([file(credentialsId: 'AWSEC2PEM', variable: 'aws_pem_file')]) {
                   sh "ansible tomcatSevers -i ansible-scripts/hosts -m ping -e ansible_ssh_private_key_file=${aws_pem_file} --ssh-common-args='-o StrictHostKeyChecking=no'"
                 }
